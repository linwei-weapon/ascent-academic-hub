...
1. `llm_enhanced` 叙事增强（协议与前端标记已预留，不改变任何数字）；LLM 供应商按决策点 D2 抽象为可切换（国内模型API/私有化），失败时角标标注"规则生成"（D3）。
2. `POST /ai/chat` 对话层：意图路由五类（查证/测算/比较/归因/开放），归因强制"事实/假设/验证"三明治，七级泛化光谱控制；卡片追问按钮接入预置 Skill 调用。
3. 学校配置中心 UI（系统管理内）：阈值/推荐问题 草稿→发布→回滚，`config_store` 版本管理与 `validate_override` 校验已就绪。
4. 黄金评估集（30-50条）与回归指标：信号命中率≥95%、Top1一致率≥90%、数字错误率=0；后端旧专家/模拟接口随评估收口统一下线。
