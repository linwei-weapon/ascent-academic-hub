...
输入：fact_grade / fact_plan_course / dim_student / fact_alert / fact_attrition
输出：sys_discovered_rule（status=pending 的建议规则）

方法：预定义可执行特征上的历史风险关联分析。结果是候选规则，不代表因果关系。
...
ALGORITHM_VERSION = "association-v2"
...
def _build_features(conn: sqlite3.Connection) -> list[dict]:
    ...
    gpa_rows = conn.execute("""
        ...
        FROM fact_grade WHERE source='real' AND gpa IS NOT NULL
    ...
    core_fail = conn.execute("""
        ...
        FROM fact_grade g
        JOIN (SELECT DISTINCT major_id,grade,course_id FROM fact_plan_course
              WHERE is_core=1) pc ON g.course_id=pc.course_id
        JOIN dim_student s ON g.student_id=s.student_id
        WHERE g.source='real' AND g.is_pass=0
          AND pc.major_id=s.major_id AND CAST(pc.grade AS TEXT)=CAST(s.grade AS TEXT)
    ...
    req_rows = conn.execute("""
        ...
        WHERE source='real' AND is_pass=1 GROUP BY student_id
    """).fetchall()
    ...
    # 真实学籍异动中的严重结果 + 已有严重预警累积，作为历史风险信号。
    adverse_students = {r["student_id"] for r in conn.execute("""
        SELECT DISTINCT student_id FROM fact_attrition
        WHERE source='real' AND kind IN ('退学','留级','肄业')
    """).fetchall()}
    repeat_rows = conn.execute("""SELECT student_id,MAX(fc) repeat_fail FROM (
        SELECT student_id,course_id,COUNT(*) fc FROM fact_grade
        WHERE source='real' AND is_pass=0 GROUP BY student_id,course_id)
        GROUP BY student_id""").fetchall()
    stu_repeat_fail = {r["student_id"]: r["repeat_fail"] for r in repeat_rows}
    fail_term_rows = conn.execute("""SELECT student_id,semester_id,
        SUM(CASE WHEN is_pass=0 THEN 1 ELSE 0 END) fc
        FROM fact_grade WHERE source='real'
        GROUP BY student_id,semester_id ORDER BY student_id,semester_id""").fetchall()
    stu_fail_terms: dict[str, list[int]] = defaultdict(list)
    for r in fail_term_rows:
        stu_fail_terms[r["student_id"]].append(r["fc"])
    ...
    for sid, s in students.items():
        ...
        # V1.1 时序特征
        # 大一挂科标记（前2学期内挂科次数）
        freshman_fail = sum(stu_fail_terms.get(sid, [])[:2])
        repeat_fail = stu_repeat_fail.get(sid, 0)
        # GPA连续下降最长序列
        consecutive_drop = 0
        cur_drop = 0
        for i in range(1, len(gpas)):
            if gpas[i] < gpas[i-1]:
                ...
                cur_drop = 0
        is_positive = sid in adverse_students or severe_alert_count >= ALERT_THRESHOLD
...
def _risk_ratio(data: list[dict], feature: str, threshold: float,
                op: str = ">=") -> tuple[float, int, int]:
    """计算满足条件的样本的风险倍数。"""
    matched = [d for d in data if (d[feature] <= threshold if op == "<="
                                   else d[feature] >= threshold)]
    if not matched:
        return 1.0, 0, 0
...
def discover() -> list[dict]:
    ...
    # (feature, threshold, op, label, name, is_integer)
    candidates = [
        ("gpa_trend", -0.3, "<=", "GPA趋势（末期-首期）", "GPA持续下降（降幅≥0.3）", False),
        ("gpa_trend", -0.5, "<=", "GPA趋势（末期-首期）", "GPA大幅下降（降幅≥0.5）", False),
        ("gpa_drop_count", 2, ">=", "GPA下滑次数", "GPA多次下滑（≥2次）", True),
        ("total_fail", 4, ">=", "挂科门次", "挂科累积（≥4门）", True),
        ("total_fail", 6, ">=", "挂科门次", "挂科严重累积（≥6门）", True),
        ("core_fail", 1, ">=", "核心课挂科", "核心课挂科（≥1门）", True),
        ("credit_ratio", 0.75, "<=", "学分完成率", "学分缺口过大（完成率≤75%）", False),
        ("freshman_fail", 1, ">=", "前两学期挂科", "前两学期挂科（≥1门次）", True),
        ("repeat_fail", 2, ">=", "同课反复挂科", "同一门课反复挂科（≥2次）", True),
        ("consecutive_drop", 3, ">=", "GPA连续下降次数", "GPA连续下降（≥3次）", True),
    ]

    rules = []
    seen_names = set()

    for feat, threshold, op, label, name, is_int in candidates:
        ratio, sample, pos_count = _risk_ratio(features, feat, threshold, op)
        ...
        # 整数字段向上取整呈现，避免"≥3.5门"
        display_value = int(threshold) if is_int else (threshold * 100 if feat == "credit_ratio" else threshold)
            ...
            rules.append({
                ...
                "conditions": [
                    {"key": feat, "label": label, "op": display_op,
                     "value": display_value, "engineValue": threshold, "unit": unit}
                ],
                "level": level,
                ...
                "detail_json": json.dumps({
                    ...
                    "outcome_definition": "真实严重学籍异动或累计3次以上当前严重预警",
                    "evidence_level": "real-derived",
                }, ensure_ascii=False),
            })
    ...
    combos = [
        (lambda d: d["gpa_trend"] < -0.3 and d["core_fail"] >= 1,
         "GPA持续下降 + 核心课挂科",
         [{"key": "gpa_trend", "label": "GPA趋势（末期-首期）", "op": "≤", "value": -0.3, "unit": ""},
        ...
        (lambda d: d["total_fail"] >= 4 and d["credit_ratio"] < 0.75,
         ...
         [{"key": "total_fail", "label": "累计挂科", "op": "≥", "value": 4, "unit": "门"},
          {"key": "credit_ratio", "label": "学分完成率", "op": "<", "value": 75,
           "engineValue": 0.75, "unit": "%"}]),
...
def run_and_save(semester_id: str = "2025-2026-2"):
    ...
    # 兼容早期分析库：自发现表曾缺少来源与审核时间字段。
    columns = {r["name"] for r in conn.execute("PRAGMA table_info(sys_discovered_rule)")}
    for name, definition in {
        "detail_json": "TEXT", "status": "TEXT DEFAULT 'pending'",
        "source": "TEXT DEFAULT 'association'", "created_at": "TEXT",
        "approved_at": "TEXT",
    }.items():
        if name not in columns:
            conn.execute(f"ALTER TABLE sys_discovered_rule ADD COLUMN {name} {definition}")

    # 保留历史审计：旧待审核建议标记为已被新一轮分析替代，不物理删除。
    ...
    for r in rules:
        ...
        existing = conn.execute(
            """SELECT id FROM sys_discovered_rule WHERE conditions=?
                AND status IN ('adopted','approved')""",
            (cond_str,)).fetchone()
        if existing:
            continue
    ...
    adopted = conn.execute("""SELECT COUNT(*) c FROM sys_discovered_rule
        WHERE status IN ('adopted','approved')""").fetchone()[0]
    print(f"规则自发现完成：{len(rules)} 条候选 → {pending} 条待审核，已采纳 {adopted} 条")
    conn.close()
    return len(rules)
