...
const utils = loadTs('src/utils/expertResearch.ts', { './http': { getToken: () => 'token', getActiveIdentity: () => 'identity' } }, { fetch: async () => { throw new Error('Unexpected request') } })
...
await test('Business result presents critical issues and scope receipt before tables', () => {
  const code = read('src/views/admin/reports/expert-team/ResearchResult.vue')
...
await test('Frozen memo body precedes collapsible detail appendix', () => {
  const code = read('src/views/admin/reports/expert-team/MaterialPreview.vue')
...
await test('Long research titles are clamped without truncating original turn messages', () => {
  const code = read('src/views/admin/reports/expert-team/index.vue')
...
const snapshots = loadTs('src/views/admin/reports/expert-team/researchSnapshot.ts')
const readingBehavior = loadTs('src/views/admin/reports/expert-team/researchReading.ts')
const layout = loadTs('src/views/admin/reports/expert-team/researchLayout.ts')
...
await test('Result arrival follows the first or waiting-at-end discussion within its reader only', () => {
  ...
  assert.doesNotMatch(read('src/views/admin/reports/expert-team/researchReading.ts'), /scrollIntoView|window\.scroll/)
})
...
const controller = loadTs('src/views/admin/reports/expert-team/useResearchWorkspace.ts', { vue: fakeVue, '@/store/auth': { authStore }, '@/utils/expertResearch': { ...utils, researchApi: api }, './researchSnapshot': snapshots }, { setTimeout: callback => { const id = nextTimer++; timerCallbacks.set(id, callback); return id }, clearTimeout: id => timerCallbacks.delete(id), window: { addEventListener() {}, removeEventListener() {} } })
...
await test('Expert cards keep details accessible and separate prepare from execute', () => {
  const code = read('src/views/admin/reports/expert-team/ExpertPicker.vue')
  ...
  assert.match(read('src/views/admin/reports/expert-team/index.vue'), /@click="newResearchDialog"/)
})
scope.stop()
await test('Leadership scope is above reading and uses unambiguous cohort labels', () => {
  const page = read('src/views/admin/reports/expert-team/index.vue')
  assert.ok(page.indexOf('<ResearchScopeSelector') < page.indexOf('ref="reading"'))
  const selector = read('src/views/admin/reports/expert-team/ResearchScopeSelector.vue')
...
await test('Leadership page keeps archived reading and normal-state copy clean', () => {
  ...
  assert.doesNotMatch(read('src/views/admin/reports/expert-team/MaterialPreview.vue'), /历史模板材料|研究编号：|材料版本：|重新校验当前权限/)
})
await test('Medium-screen notes free reading space without rewriting sidebar preference', () => {
  ...
  assert.match(read('src/views/admin/reports/expert-team/ResearchSidePanel.vue'), /@keydown.esc.stop="\$emit\('close'\)"/)
  const layout = read('src/views/admin/Layout.vue')
...
await test('New workspace stores no business draft in browser persistent storage', () => {
  const code = read('src/views/admin/reports/expert-team/useResearchWorkspace.ts')
...
await test('Reading area takes priority and the composer expands explicitly', () => {
  ...
  const styles = read('src/views/admin/reports/expert-team/leadership.css')
...
await test('New results use a non-overlapping reading locator and separate data view', () => {
  ...
  assert.match(read('src/views/admin/reports/expert-team/ResearchResult.vue'), /v-show="view === 'data'"/)
})
await test('Research progress uses the supplied management summary while retaining numeric result content', () => {
  assert.match(read('src/views/admin/reports/expert-team/index.vue'), /result\.decision_summary \|\| result\.headline/)
  assert.match(read('src/views/admin/reports/expert-team/ResearchResult.vue'), /result\.headline \|\| result\.title/)
})
for (const name of ['index.vue', 'ResearchScopeSelector.vue', 'ResearchResult.vue', 'ResearchSidePanel.vue', 'ExpertPicker.vue', 'ExpertAvatar.vue', 'MaterialPreview.vue']) {
  await test(`Vue component parses and template compiles: ${name}`, () => {
    const filename = path.join(base, 'src/views/admin/reports/expert-team', name), source = fs.readFileSync(filename, 'utf8')
