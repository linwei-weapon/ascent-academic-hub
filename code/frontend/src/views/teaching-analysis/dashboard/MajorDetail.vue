<!-- 教学数据总览：MajorDetail 页面或专用组件，保留原业务与权限行为。 -->
<template>
...
async function loadData() {
  ...
  try {
    ...
    const d = await dashboardApi.getMajorOverview((route.params.id || 'M051'), qs)
    if (d) {
