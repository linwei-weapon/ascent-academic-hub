<!-- 培养质量分析：GraduationReadiness 页面或专用组件，保留原业务与权限行为。 -->
<template>
  <div>
        ...
        <div class="table-horizontal-scroll">
          <div class="course-table-width">
            <AppTable :columns="courseColumns"  storage-key="curriculum:graduation-courses"
              ...
              <template #col-action="{row}"><el-button link type="primary" @click="openSupply(row)">详情</el-button></template>
            </AppTable>
          </div>
        </div>
      </section>
    ...
    <section ref="studentSection" class="sa-card student-section">
      ...
      <AppTable :columns="studentColumns" :data="data.students" storage-key="curriculum:graduation-students"
        :max-business-columns="10" :config-version="3"
          stripe
    ...
    <el-drawer v-model="studentVisible" :title="studentEvidence.student?.display_name ? `${studentEvidence.student.display_name}｜毕业准备核查证据` : '毕业准备核查证据'" size="900px">
      <div v-loading="studentDrawerLoading" element-loading-text="正在加载学生模块与课程证据…" class="drawer-body">
        <el-descriptions class="drawer-summary" :column="3" border>
          ...
          <el-descriptions-item label="院系">{{studentEvidence.student?.organization_name || '—'}}</el-descriptions-item>
          <el-descriptions-item label="专业">{{studentEvidence.student?.major_name || '—'}}</el-descriptions-item>
          <el-descriptions-item label="行政班">{{studentEvidence.student?.class_code || '—'}}</el-descriptions-item>
    ...
    <el-drawer v-model="supplyVisible" :title="supply.course?.courseName || '课程保障证据'" size="860px">
...
const studentColumns:AppTableColumn[]=[
  ...
  {key:'organization_name',label:'院系',minWidth:160,tooltip:true},
  {key:'major_name',label:'专业',minWidth:145},
  {key:'class_code',label:'行政班',minWidth:150,tooltip:true},
  {key:'moduleProgress',label:'已完成/可核查模块',minWidth:145},
  {key:'completed_credits',label:'已完成学分',minWidth:100,align:'center'},
  ...
  {key:'evidence',label:'核查重点',minWidth:260,required:true,tooltip:true},
...
const supplyStudentColumns:AppTableColumn[]=[
  ...
  {key:'collegeName',label:'院系',minWidth:160,tooltip:true},
  {key:'majorName',label:'专业',minWidth:150},
  {key:'action',label:'操作',width:110,fixed:'right',required:true,region:'action'},
]
const kpis=computed(()=>[
...
async function loadFilterOptions(){
  ...
  try{
    const options=await curriculumApi.getGraduationOptions<any>()
...
// 将原分页和已应用条件交给业务 API，指标计算仍由服务端完成。
function requestData(){const q=appendGlobalParams(new URLSearchParams({limit:String(pageSize.value),offset:String((page.value-1)*pageSize.value)}));if(status.value)q.set('readiness',status.value);return curriculumApi.getGraduationReadiness<any>(q)}
