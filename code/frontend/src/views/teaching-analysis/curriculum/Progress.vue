<!-- 培养质量分析：Progress 页面或专用组件，保留原业务与权限行为。 -->
<template>
  <div>
          ...
          <el-tabs v-model="detailTab">
            <el-tab-pane name="modules">
              ...
              <AppTable :columns="moduleColumns" :data="moduleProgress"
                ...
                :config-version="3" :show-density="true" :show-column-settings="true" :pagination="false">
...
const studentColumns:AppTableColumn[]=[
  ...
  {key:'moduleProgress',label:'已完成/可核查模块',minWidth:145,required:true},
  {key:'earnedCredits',label:'已完成学分',minWidth:105,align:'center'},
...
async function load(planId:string){
  ...
  loading.value=true
  try{const data=await curriculumApi.getPlanProgress<any>(planId);planName.value=data.plan?.planName||planId;planVersion.value=data.plan?.planId||'';progress.value=data.students||[];summary.value=data.summary||{};definition.value=data.definition||{}}
  finally{loading.value=false}
}
// 沿用当前学生与查询上下文打开证据入口，保留原返回路径。
...
async function loadStudentDetail(){
  ...
  try{const d=await curriculumApi.getProgressStudentCourses<any>(selectedStudent.value.studentId);detailCourses.value=d?.items||[];moduleProgress.value=d?.moduleProgress||[]}
