<!-- 培养质量分析：index 页面或专用组件，保留原业务与权限行为。 -->
<template>
...
async function loadOverview(force=false){
  ...
  try{Object.assign(overview,await curriculumApi.getCurriculumOverview(overviewQueryString()));overviewLoaded.value=true}
  finally{overviewLoading.value=false}
  if(!courseLoaded.value||force){
    courseLoading.value=true
    overview.bottleneckCourses=[]
    try{const result=await curriculumApi.getCurriculumCourses(overviewQueryString({limit:'20'}));overview.bottleneckCourses=result.items||[];courseLoaded.value=true}
...
async function loadStudentPage(){
  ...
  try {
    ...
    const data=await curriculumApi.getCurriculumStudents(query.toString())
...
async function studentProfile(row:any) {
  ...
  try { studentEvidence.data=await curriculumApi.getCurriculumStudentEvidence(row.studentId) }
...
async function loadPlan() {
  ...
  try {
    const d = await curriculumApi.getCurriculumPlan(selectedMajor.value)
    if (d?.plan) {
      Object.assign(plan, {
...
onMounted(async () => {
  ...
  try {
    const d = await curriculumApi.getCurriculumOptions()
