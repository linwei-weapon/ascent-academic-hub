<!-- 培养质量分析：Governance 页面或专用组件，保留原业务与权限行为。 -->
<template>
  <div>
...
async function load(){loading.value=true;try{const [d,o]=await Promise.all([curriculumApi.getCurriculumRuleChanges<any>(),curriculumApi.getCurriculumRuleOptions<any>()]);rows.value=d.list||[];Object.assign(permissions,d.permissions||{});Object.assign(options,o||{})}finally{loading.value=false}}
async function create(){const [majorId,grade]=planParts.value;let payload:any
  ...
  await curriculumApi.createCurriculumRuleChange({ruleType:form.ruleType,payload,reason:form.reason});dialog.value=false;form.reason='';await load()}
async function act(row:any,action:string){await ElMessageBox.confirm(action==='activate'?'激活后将立即参与学生合规计算，确认继续？':'确认提交质量审核？','确认');await curriculumApi.actOnCurriculumRuleChange(row.change_id, action, {});await load()}
async function review(row:any,approved:boolean){const r=await ElMessageBox.prompt('请输入审核意见','质量审核',{inputValue:approved?'审核通过':'资料或规则需修订'});await curriculumApi.reviewCurriculumRuleChange(row.change_id, {approved,comment:r.value});await load()}
async function detail(row:any){current.value=await curriculumApi.getCurriculumRuleChange(row.change_id);detailDialog.value=true}
async function preview(row:any){Object.assign(previewData,await curriculumApi.previewCurriculumRuleChange(row.change_id));previewDialog.value=true}
