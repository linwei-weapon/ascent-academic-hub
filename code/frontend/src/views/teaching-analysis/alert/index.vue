<!-- 学业预警监控：index 页面或专用组件，保留原业务与权限行为。 -->
<template>
...
async function loadOptions(source = draft) {
  ...
  if (source.grade) params.set('grade', source.grade)
  if (source.classId) params.set('class_id', source.classId)
  const response: any = await alertApi.getAlertOptions(params)
...
async function loadAll() {
  ...
  try {
    ...
    const [summaryData, studentData, distributionData, timeResult, priorityData] =
      await Promise.all([
        alertApi.getAlertSummary<any>(params),
        alertApi.getAlertStudents<any>(studentParams()),
        alertApi.getAlertDistribution<any>(distributionParams),
        alertApi.getAlertTimeDistribution<any>(params),
        alertApi.getAlertPriority<any>(priorityParams),
      ])
...
async function fetchStudentPage(preset: typeof cardPreset) {
  ...
  loadError.value = ''
  try {
    const data: any = await alertApi.getAlertStudents(studentParams(preset))
    if (sequence !== requestSequence.value) return
...
async function exportCurrentList() {
  ...
  try {
    ...
    const response = await alertApi.exportAlertStudents(params)
    if (!response.ok) {
      let message = '导出失败'
      try {
...
.priority-card {
  ...
  transition: .18s ease;
  &:hover {
    box-shadow: 0 5px 16px rgba(190, 24, 93, .1);
  }
  > div {
    display: flex;
    justify-content: space-between;
...
@media (max-width: 760px) {
  .kpi-grid, .priority-grid, .monitor-context .filter-grid {
    grid-template-columns: 1fr;
  }
