<!-- 学业预警监控：AlertStudentDrawer 页面或专用组件，保留原业务与权限行为。 -->
<template>
  <el-drawer
...
async function load() {
  ...
  try {
    const [studentResult, workflowResult] = await Promise.allSettled([
      alertApi.getAlertStudent(props.row.studentId),
      eventId ? alertApi.getAlertEvent(eventId) : Promise.resolve({}),
    ])
...
async function addFollowup() {
  ...
  saving.value = true
  try {
    await alertApi.createAlertFollowup(workflow.value.eventId, {
      action_type: '核查记录',
...
async function saveStatus() {
  ...
  saving.value = true
  try {
    await alertApi.updateAlertStatus(workflow.value.eventId, {
