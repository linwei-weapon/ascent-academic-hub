<!-- 预警规则治理：RuleGovernance 页面或专用组件，保留原业务与权限行为。 -->
<template>
  <div>
...
async function saveRule() {
  ...
  saving.value = true
  try {
    await rulesApi.createRuleChange(editingRule.value.id, { values, reason: editForm.reason.trim() })
...
async function onRuleToggle(row: Rule) {
  ...
  try {
    ...
    await rulesApi.createRuleChange(row.id, { enabled: requested, reason: value.trim() })
...
async function openTrialScript(row:RuleChange) {
  ...
  try {
    const data = await rulesApi.getRuleTrialScript<any>(row.changeId)
...
async function saveTrialScript() {
  ...
  try {
    await rulesApi.updateRuleTrialScript(scriptChangeId.value, {
...
async function loadCandidates(page=1) {
  ...
  try {
    const d = await rulesApi.getRuleCandidates<any>(candidateChangeId.value, page)
...
async function openAnalysis(row:RuleChange) {
  analysis.value = await rulesApi.getRuleAnalysis<any>(row.changeId)
  analysisVisible.value = true
}
// 下载当前变更单的候选名单，保留原鉴权头与文件处理。
async function downloadCandidates() {
  const res = await rulesApi.exportRuleCandidates(candidateChangeId.value)
...
async function loadChanges() {
  try {
    const list = await rulesApi.getRuleChanges<RuleChange[]>()
...
async function changeAction(row:RuleChange, action:string) {
  try {
    ...
    if (action === 'evaluate') await rulesApi.evaluateRuleChange(row.changeId)
    else if (action === 'submit') await rulesApi.submitRuleChange(row.changeId)
    else if (action === 'publish') await rulesApi.publishRuleChange(row.changeId)
    else if (action === 'activate') await rulesApi.activateRuleChange(row.changeId)
    else if (action === 'rollback') await rulesApi.rollbackRuleChange(row.changeId)
    else await rulesApi.reviewRuleChange(row.changeId, { action })
    ElMessage.success('操作成功')
    await loadChanges()
    if (action === 'publish') {
      const d = await rulesApi.getRules<{rules:Rule[]}>()
...
async function loadDiscovered() {
  try {
    const d = await rulesApi.getDiscoveredRules<any>()
...
async function runDiscovery() {
  ...
  try {
    const d = await rulesApi.discoverRules<any>()
...
async function reviewRule(id: number, action: string) {
  try {
    const d = await rulesApi.updateDiscoveredRule<any>(id, { action })
    ElMessage.success(d.msg || (action === 'approve' ? '已创建规则变更草稿' : '规则建议已拒绝'))
    await loadDiscovered()
    await loadChanges()
    // 同时刷新上方规则表
    const r = await rulesApi.getRules<{ rules: Rule[] }>()
...
onMounted(async () => {
  ...
  ruleLoadError.value = ''
  try {
    const p = await rulesApi.getRulePermissions<any>()
...
.analysis-cross-table {
  ...
  :deep(.analysis-collegeSubtotal td.el-table__cell) {
    background: #f8fafc !important;
    font-weight: 650;
    color: #334155;
  }
  :deep(.analysis-grandTotal td.el-table__cell) {
