<!-- 学业预警监控：EarlySetback 页面或专用组件，保留原业务与权限行为。 -->
<template>
...
async function loadOptions(filter: FilterState = draft) {
  ...
  const result = await alertApi.getEarlySetbackOptions<any>(query.toString())
...
async function load() {
  ...
  try {
    ...
    const result = await alertApi.getEarlySetback<any>(query.toString())
