<!-- 学生成长与学业分析：Detail 页面或专用组件，保留原业务与权限行为。 -->
<template>
  <div>
    <!-- 返回溯源 -->
...
onMounted(async () => {
  try {
    const d = await studentsApi.getStudentDetail(route.params.id)
  ...
  try {
    ...
    const [growth, actionable, candidates, adviceData] = await Promise.all([
      studentsApi.getStudentGrowth<any>(id),
      studentsApi.getStudentActionableCourses<any>(id),
      studentsApi.getStudentIncompleteCourses<any>(id),
      studentsApi.getStudentAdvice<any>(id),
    ])
