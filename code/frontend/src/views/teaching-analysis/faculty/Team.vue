<!-- 师资保障分析：Team 页面或专用组件，保留原业务与权限行为。 -->
<template>
  <div>
    ...
    <template v-if="course">
      ...
      <div class="grid"><section class="sa-card"><div class="sa-card-title">实际授课团队成员 <span class="extra">来源：教学任务教师关联</span></div><AppTable :data="team.members||[]" stripe :columns="[]" storage-key="teaching-analysis:faculty:team:1" :pagination="false">
        <template #columns><el-table-column prop="display_name" label="教师" min-width="110" align="center" header-align="center"/><el-table-column prop="staff_id" label="教师代码" min-width="120" align="center" header-align="center"/><el-table-column prop="title" label="职称" min-width="120" align="center" header-align="center"><template #default="{row}">{{row.title||'待补充'}}</template></el-table-column><el-table-column prop="organization_id" label="组织代码" min-width="120" align="center" header-align="center"/><el-table-column label="证据状态" min-width="105" align="center" header-align="center"><template #default="{row}"><el-tag :type="row.title?'success':'warning'">{{row.title?'职称已知':'职称缺失'}}</el-tag></template></el-table-column><el-table-column label="操作" width="90" align="center" header-align="center"><template #default="{row}"><el-button link type="primary" @click="teacher(row)">教学档案</el-button></template></el-table-column>        </template>
      </AppTable></section>
      <section class="sa-card"><div class="sa-card-title">历史开课供给证据</div><AppTable :data="supply.offerings||[]" :columns="[]" storage-key="teaching-analysis:faculty:team:2" :pagination="false">
        <template #columns><el-table-column prop="semesterId" label="学期" min-width="115" align="center" header-align="center"/><el-table-column prop="lessonCount" label="教学班" min-width="75" align="center" header-align="center"/><el-table-column prop="teacherCount" label="教师" min-width="70" align="center" header-align="center"/><el-table-column prop="capacity" label="容量" min-width="75" align="center" header-align="center"/><el-table-column prop="enrolled" label="选课人次" min-width="85" align="center" header-align="center"/><el-table-column prop="avgClassSize" label="平均班额" min-width="85" align="center" header-align="center"/>        </template>
      </AppTable></section></div>

      <section class="sa-card"><div class="sa-card-title">管理核查结论</div><AppTable :data="checks" stripe :columns="[]" storage-key="teaching-analysis:faculty:team:3" :pagination="false">
        <template #columns><el-table-column prop="level" label="级别" min-width="85" align="center" header-align="center"><template #default="{row}"><el-tag :type="row.level==='优先'?'danger':row.level==='核验'?'warning':'info'">{{row.level}}</el-tag></template></el-table-column><el-table-column prop="topic" label="核查事项" min-width="160" align="center" header-align="center"/><el-table-column prop="basis" label="事实依据" min-width="250" align="center" header-align="center"/><el-table-column prop="action" label="建议管理动作" min-width="300" align="center" header-align="center"/>        </template>
      </AppTable></section>
...
import{computed,onMounted,reactive,ref}from'vue';import{useRoute,useRouter}from'vue-router';;import{getV2TeachingSemester}from'@/utils/v2meta'
...
async function search(){if(!query.value.trim())return;searching.value=true;try{results.value=await facultyApi.searchFacultyTeam<any>(query.value)}finally{searching.value=false}}
...
async function load(id:string){const r=await facultyApi.getTeamCourse<any>(id, semester.value);Object.assign(team,{summary:r.summary,members:r.members});Object.assign(supply,{offerings:r.offerings,substitutions:[],availability:{hasFuturePlanEvidence:false},boundary:r.boundary})}
