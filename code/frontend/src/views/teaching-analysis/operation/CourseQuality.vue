<!-- 教学运行分析：CourseQuality 页面或专用组件，保留原业务与权限行为。 -->
<template><div>
...
async function load() {
  ...
  try {
    ...
    const r = await operationApi.getCourseQuality<any>(q);
...
async function selectCourse(row: any) {
  ...
  try {
    ...
    if (from.value) q.set('semester_from', from.value);
    if (to.value) q.set('semester_to', to.value);
    const r = await operationApi.getCourseQualityDetail<any>(row.course_id, q);
