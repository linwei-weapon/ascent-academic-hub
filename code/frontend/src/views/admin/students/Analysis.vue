...
const organizationCols = computed<DataTableColumn[]>(() => [
...
const studentCols: DataTableColumn[] = [
  ...
  { key: 'openAlerts', label: '当前有效预警', width: 110, align: 'right' },
...
.comparison-board { display:grid; grid-template-columns:minmax(240px,1.45fr) minmax(230px,1.35fr) repeat(3,minmax(160px,1fr)); margin-bottom:14px; overflow:hidden; border:1px solid #dbeafe; border-radius:6px; background:#fff; box-shadow:0 3px 12px rgba(37,99,235,.06); }
.comparison-board-item { min-width:0; min-height:58px; padding:0 16px; display:flex; align-items:center; justify-content:space-between; gap:14px; border:0; border-right:1px solid #e2e8f0; background:#fff; color:#475569; text-align:left; box-sizing:border-box; }
.comparison-board-item:last-child { border-right:0; }
.comparison-board-item span { flex:none; font-size:12px; color:#64748b; }
.comparison-board-label { display:inline-flex; align-items:center; gap:4px; }
.comparison-board-label .el-icon { color:#94a3b8; cursor:help; }
.comparison-board-item strong { min-width:0; color:#172033; font-size:13px; text-align:right; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.comparison-board-title strong { color:#1e293b; }
.comparison-board-action { cursor:pointer; font:inherit; }
.comparison-board-action strong { color:#4f46e5; text-decoration:underline; text-underline-offset:3px; }
.comparison-board-action:hover { background:#f5f3ff; }
.comparison-board-action:focus-visible { position:relative; z-index:1; outline:2px solid #6366f1; outline-offset:-2px; }
.comparison-board-action:disabled { cursor:default; }
.comparison-board-action:disabled strong { color:#94a3b8; text-decoration:none; }
...
.organization-panel { width:100%; box-sizing:border-box; }
