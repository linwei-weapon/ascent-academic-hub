  ...
  <div>
        ...
        <div class="table-horizontal-scroll">
          <div class="course-table-width">
            <DataTable :columns="courseColumns" :data="data.courses" storage-key="curriculum:graduation-courses"
              :max-business-columns="4" :config-version="3" :pagination="true" :default-page-size="10"
              size="small" v-loading="initialLoading">
    ...
    <section ref="studentSection" class="sa-card student-section">
      ...
      <DataTable :columns="studentColumns" :data="data.students" storage-key="curriculum:graduation-students"
        :max-business-columns="10" :config-version="3" :page-size="pageSize"
    ...
    <el-drawer v-model="supplyVisible" :title="supply.course?.courseName || '课程保障证据'" size="860px">
      <div v-loading="supplyDrawerLoading" element-loading-text="正在加载历史开课、替代和影响学生证据…" class="drawer-body">
        ...
        <h4>受影响学生</h4>
        <DataTable :columns="supplyStudentColumns" :data="supplyStudents" storage-key="curriculum:graduation-supply-students" :max-business-columns="3" :config-version="3" size="small" v-loading="supplyStudentsLoading">
...
const studentColumns:DataTableColumn[]=[
  ...
  {key:'moduleProgress',label:'已完成/可核查模块',width:145},
  {key:'completed_credits',label:'已完成学分',width:100,align:'right'},
...
const evidenceCourseColumns:DataTableColumn[]=[
  ...
  {key:'reason',label:'原因&建议',minWidth:280,required:true,tooltip:true},
]
const candidateCourseColumns:DataTableColumn[]=[
  {key:'course_name',label:'课程',minWidth:170,fixed:'left',required:true,region:'identity'},
  {key:'module',label:'模块',minWidth:130},{key:'module_rule',label:'模块规则',minWidth:160},
  {key:'suggested_term',label:'建议学期',width:90},{key:'reason',label:'原因&建议',minWidth:300,required:true,tooltip:true},
]
