        ...
        <div class="change-grid">
          <article v-for="(item,index) in data.changes || []" :key="item.theme" class="change-card" :class="{primary:index===0}">
            ...
            <div class="card-action">
              <el-button @click="go(item.route,item.routeQuery)">查看事实证据</el-button>
              <el-button v-if="item.expertId" type="primary" @click="continueReview(item)">继续研判</el-button>
            </div>
          </article>
        </div>
      </section>
      ...
      <section class="sa-card ongoing-card">
        ...
        <el-table :data="data.ongoingPriorities || []" stripe>
          ...
          <el-table-column label="操作" width="190" fixed="right"><template #default="{row}">
            <el-button link @click="go(row.route,row.routeQuery)">事实证据</el-button>
            <el-button v-if="row.expertId" link type="primary" @click="continueReview(row)">继续研判</el-button>
...
function continueReview(item:any){router.push({path:'/admin/reports/decision-simulation',query:{
  expert:item.expertId,event:item.rootKey||item.theme,
  question:item.reviewQuestion||`继续研判：${item.theme}`,
  semester:data.semester?.teaching||data.semester?.grade||'',
}})}
