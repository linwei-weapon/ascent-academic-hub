<!-- 分析方案管理：系统管理模块，沿用既有接口、治理操作与权限边界。 -->
<template>
        ...
        <el-form label-width="190px" class="scheme-form">
          ...
          <el-form-item v-for="key in boundKeys(editSkill)" :key="key" :label="fieldMeta(key).label">
            ...
            <el-select v-else-if="editSkill.config_bounds[key].type==='list'" v-model="form.values[key]"
              multiple filterable allow-create default-first-option class="scheme-list-control" />
...
.summary-grid {
  ...
  button:hover, button.active {
    border-color: #6366f1;
    box-shadow: 0 5px 16px rgba(79,70,229,.09);
  }
...
.parameter-row,.readiness-row {
  display: flex;
  justify-content: space-between;
  align-items: center;
  gap: 16px;
  padding: 10px 0;
  border-bottom: 1px dashed var(--sa-border-2);
}
...
@media (max-width:720px) {
  .summary-grid,.template-grid,.fact-grid,.check-grid {
