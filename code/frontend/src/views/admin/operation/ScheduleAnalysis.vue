<template>
  <div>
    ...
    <div class="sa-card" style="margin-bottom:16px">
      ...
      <el-table :data="data.focusSummary" size="small">
        <el-table-column prop="group" label="课程组" width="100" />
        <el-table-column prop="courseCount" label="课程门数" width="90" align="right" /><el-table-column prop="lessonCount" label="教学班" width="82" align="right" /><el-table-column prop="meetingCount" label="排课片段" width="90" align="right" /><el-table-column prop="studentVisits" label="学生人次" width="95" align="right" />
        <el-table-column prop="peakSlot" label="最集中时段" width="120" /><el-table-column label="峰值占比" width="96" align="right"><template #default="{row}"><span :class="row.peakShare>=20?'warn':''">{{ row.peakShare }}%</span></template></el-table-column>
        <el-table-column label="晚间占比" width="96" align="right"><template #default="{row}">{{ row.eveningShare }}%</template></el-table-column>
        <el-table-column label="操作" min-width="90"><template #default="{row}"><el-button text type="primary" @click="openGroup(row.group)">查看课程</el-button></template></el-table-column>
      </el-table>
    </div>
