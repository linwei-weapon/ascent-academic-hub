...
import { researchApi, ResearchApiError } from '@/api/aiDecision/expertResearch'
import { newRequestId, isRunning, comparableResearchPlans, clearAcceptedText, appendWithoutOverwrite } from '@/utils/expertResearch'
import type { Research, ResearchCatalog, ResearchSummary, ResearchScope, ResearchMaterial, ResearchSources, ResearchResult, ResearchQuestion, ResearchExpert, SavedText, SaveTextBody } from '@/types/expertResearch'
...
export function useResearchWorkspace() {
  ...
  async function saveField(id: string, field: FieldName): Promise<boolean> {
    ...
    try {
      ...
      // await 期间轮询可能写入新冲突，按原声明类型读取，避免沿用发送前的 null 收窄。
      if (saved.revision < Math.max(state.base.revision, (state.conflict as SavedText | null)?.revision || 0, records[id]?.[field].revision || 0)) {
        state.pending = null
