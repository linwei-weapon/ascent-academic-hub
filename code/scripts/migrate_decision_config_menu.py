"""决策配置菜单迁移：/admin/system/decision-config 挂到 系统管理 分组并授权 dean。

幂等：只插入/更新 sys_menu 一行并补齐 sys_role_menu 授权，不触碰其他菜单。

运行（仓库根目录）：
    python -X utf8 code/scripts/migrate_decision_config_menu.py
"""
import sqlite3
import sys
...
MENU = ("/admin/system/decision-config", "/admin/system", "决策配置",
        "/admin/system/decision-config", "MagicStick", 309)
ROLES = ("dean",)  # 系统管理分组惯例：仅系统管理员
