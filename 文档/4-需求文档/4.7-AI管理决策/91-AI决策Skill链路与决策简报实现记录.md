...
1. ~~学校配置中心 UI~~（已完成，见 §10）
2. ~~黄金评估集（30-50条）与回归指标~~（已完成，见 §11）：信号命中率≥95%、Top1一致率≥90%、数字错误率=0；后端旧专家/模拟接口随评估收口统一下线。
3. 可选增强：真实 token 级流式（需配套在线增量数字校验）；归因假设层的多假设排序；测算类 Skill。

## 10. 阶段5：学校配置中心（2026-07-22）

学校可调的决策参数全部收口到「系统管理 → 决策配置」（仅系统管理员，`require_admin` 守卫）。

### 10.1 配置治理口径

- **白名单边界**：学校覆写只允许落在各 Skill `config_bounds` 声明的范围内（类型/上下限校验），公式、数据来源、证据路由不可覆写——这是协议层 `validate_override` 的硬约束，端点只做转述。
- **三态流转**：草稿（draft）→ 发布（published，旧版自动退役 retired）→ 回滚（以历史版本内容为蓝本生成新发布版本，审计链只增不删）。
- **最小覆写**：前端只提交与产品默认不同的键，默认值演进时学校未覆写的项自然跟随。
- **配置生效路径**：发布后下一次 Skill 运行即按新配置产出信号（简报指纹随信号变化自动失效重算，无需手工清缓存）。

### 10.2 端点（`/api/admin/ai/decision/config/*`）

| 端点 | 用途 |
|---|---|
| `GET /config/skills` | 全部Skill：默认值/边界/生效覆写/版本链 |
| `POST /config/skills/{id}/draft` | 创建草稿（越界/非白名单 400，错误信息含具体原因） |
| `POST /config/skills/{id}/publish` | 发布草稿 |
| `POST /config/skills/{id}/rollback` | 回滚到历史版本 |
| `GET /config/llm` | LLM配置（api_key 脱敏：只回 has_api_key + 尾4位） |
| `PUT /config/llm` | 保存（api_key 缺省=保持原密钥，空串=清除） |
| `POST /config/llm/test` | 连通性测试（用已保存配置试调，不要求 enabled；失败返回 kind+detail） |

### 10.3 前端页面（`system/DecisionConfig.vue`）

- **LLM 增强配置卡**：enabled 开关、base_url/model/api_key（不回显，尾号提示）、超时/重试（上限1次，防线硬约束）、叙事与对话独立开关；保存 + 测试连接（成功显示模型名，失败显示错误类别）。
- **Skill 阈值配置卡**：按 Skill 分 Tab，数值型按 bounds 渲染 input-number（min/max/precision），列表型渲染可创建多选；每项标注「默认 · 当前生效 · 可调范围」；变更原因必填（进审计链）；保存草稿 / 发布草稿 / 还原为生效值。
- **版本历史抽屉**：版本号/状态/变更原因/创建与发布人及时间，草稿可发布、已发布与已退役可回滚（回滚必填原因）。
- 菜单「系统管理 → 决策配置」（sort 309，仅 dean 授权；seed + `migrate_decision_config_menu.py` 幂等迁移已执行）。

### 10.4 验证记录（阶段5）

- 单测 14 例（`test_decision_config.py`）：清单结构、草稿→发布生效、越界拒绝、非白名单拒绝、重复发布拒绝、回滚审计链（3版本链）、404、密钥脱敏/保持/清除、连通性测试成败路径、require_admin 守卫。全套 **144 例** 全绿。
- 端到端（`work/verify_stage5.py`，17 项）：真实服务上完成 越界400/非白名单400/草稿不生效/发布生效/简报反映新版本/回滚/脱敏/未配齐诚实失败/菜单可见，验证后现场恢复产品默认。
- 前端 `npm run build` 通过。

## 11. 阶段6：黄金评估集与旧接口下线（2026-07-22）

里程碑B 收口：用黄金集把「数字真实」从承诺变成可回归的指标，同时下线被 Skills 取代的旧专家/模拟接口。

### 11.1 旧专家/模拟接口下线（决策点 D1）

- **边界探查结论**：`routers/ai.py` 的 `/admin/ai/insight/*` 学生洞察端点是**活代码**（前端 `ai.ts` 消费），依赖旧 `routers/ai_experts.py` 的建表/版本解析 helper；`Schemes.vue`（分析方案管理，活页面）消费 3 个写端点。故不能整包删除，只做**路由下线、能力迁移**。
- **helper 下沉**：新建 `backend/ai_experts/versions.py`（建表 DDL、`active_version`、`resolve_effective_expert` 等），`routers/ai.py` 与 `routers/system_management.py` 改为从这里导入；`backend/ai_experts` 包本身（协议校验、样例数据）保留。
- **端点迁移**：方案/版本的 3 个写端点迁入 `system_management.py`——`POST /api/admin/system/analysis-schemes/{id}/publish`、`POST /api/admin/system/expert-versions/{id}/publish|rollback`；`Schemes.vue` 三个 URL 同步改到新路径。
- **删除**：`routers/ai_experts.py` 整文件删除，`main.py` 摘除路由挂载。旧路径全部 404/405。

### 11.2 黄金评估集（37 例）

`tests/golden/decision_golden.v1.json` + `test_decision_golden.py`，对真实演示库运行（库缺失自动 skip）。用例先由 `work/probe_golden.py` 探测真实信号现状、再人工核验固化；数据或阈值变更导致合理漂移时需人工复核后更新基线文件。

- **信号用例 13 条**：覆盖 4 个 Skill 的代表性信号（毕业缺口 4 / 课程质量 4 / 预警优先级 2 / 师资结构 3），断言信号ID存在 + 严重度/类型一致 + 关键数字命中（如体质测试受阻 51 人、2022 级受阻 144 人、滞留预警 950 条、军事理论修读 2404 人）。
- **Top1 用例 3 条**：简报优先处置首位 = 体质测试（critical）、Top1 来自 main 层 Skill、跨专题热点成对出现（军事理论 毕业缺口×师资结构）。
- **对话题例 21 条**：查证 7（含 1 条边界题：无命中信号时必须诚实回答「没有与该问题直接相关」）、归因 5（强制事实/假设·待验证/行动三层且事实层含关键数字）、比较 3、假设测算 3（必须诚实声明「后续阶段」能力边界）、开放 3。
- **指标口径**：信号命中率 ≥95%；Top1 一致率 ≥90%；**对话数字错误率 = 0**——答案中全部数字字面量（剥离列表序号后）必须可追溯到引用信号载荷、问题本身或简报 topline。

### 11.3 验证记录（阶段6）

- 黄金评估 **5/5 通过**：信号命中率 13/13=100%、Top1 一致率 3/3=100%、对话数字错误率 0（21 条题例全部数字可追溯）、信号ID确定性回归（同库重跑两次 ID 集合一致）。
- 后端全套 **146 例** 全绿（下线删除 3 例旧路由测试后）。
- 端到端下线回归（`work/verify_stage6_e2e.py`，9/9）：旧 `/ai/experts` 系列 404/405；保留的 `/admin/ai/insight/alert-summary` 正常；迁入的 `/system/analysis-schemes`（含 versions 字段）正常；决策简报/llm-status 正常；SSE 追问实测 meta→delta→done 完整（verify 意图数字直查、followups 与 llm_status 随 done 返回）。
- 前端 `npm run build` 通过。

> 里程碑B（阶段0→6）至此全部完成：Skills 信号 → 合并简报 → 快照diff → 建议追踪 → LLM 编排（可开关、失败诚实标注）→ 学校配置治理 → 黄金集回归与旧接口下线。
