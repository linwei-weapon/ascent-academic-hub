...
各层级只改变基础人群，不改变指标公式。覆盖率分母在历史学期使用源学期库同层级学生名单，当前统计学期使用当前总览卡片同一名单；当前统计学期的“较上学期”覆盖率变化额外使用当前名单重算上一学期比较基线，确保与首页卡片一致，但不覆盖上一历史学期自身的单点值。挂科学生率分母使用同层级当学期有有效成绩的学生；平均 GPA 只对同层级当学期有加权 GPA 的学生求平均。历史源库文件缺失、`students` 表缺失或组织名称无法映射时，该历史学期返回不可用原因，不回退使用当前 `dim_student`。
...
| DEC-15 | 当前覆盖率环比 | 历史弹窗当前统计学期的有效成绩覆盖率环比，使用当前授权在籍名单重算上一学期覆盖率作为比较基线，与首页卡片同口径；上一历史学期自身单点仍使用该学期源名单，不改写历史值 |
