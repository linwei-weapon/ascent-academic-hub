...
- **REQ-005**: 表格中的学年和学期合并为一个“学年学期”字段，例如 `2025-2026学年 第2学期`，不拆成两个低信息量字段。
...
| 顺序 | 字段 | 说明 |
|---:|---|---|
| 1 | 学年学期 | 合并显示，例如“2025-2026学年 第2学期” |
| 2 | 指标值 | 百分比，保留 1 位小数 |
| 3 | 较上学期 | 上升/下降百分点；不可比显示“—”及原因 |
| 4 | 分子 | 按指标显示“覆盖人数/挂科人数/预警人数/通过人次” |
| 5 | 分母 | 按指标显示“在籍人数/有效成绩人数/考试人次” |
| 6 | 数据状态 | 真实、派生、按当前规则回算、暂不可用 |
...
表格使用 `DataTable`，学年学期和指标值为必选列，默认按学期倒序；图表按学期正序。

#### 1.5.7 图表规范

- 比率图：单学期只有一个堆叠柱，柱内“分子 + 分母减分子”等于总分母；比率折线使用右轴。
- 覆盖率/通过率：分子使用 `--sa-teal`，剩余使用 `--sa-track`，折线使用 `--sa-primary`。
- 挂科率/预警率：分子使用 `--sa-danger`，剩余使用 `--sa-track`，折线使用 `--sa-danger`。
- GPA：只显示 `--sa-primary` 折线和数据点，不添加无对应量纲的人数柱。
- Tooltip 同时显示学年学期、分子、分母、指标值、环比和数据状态。
...
| TASK-002 | 将 DEC-01～DEC-10 的确认结果写入本文件，冻结指标名称、公式、分子、分母、数据范围、排序和空值规则。 | ✅ | 2026-07-27 |
