...
各层级只改变基础人群，不改变指标公式。每个学期的有效成绩覆盖率均使用“该学期有效成绩人数÷该学期在籍人数”；历史学期在籍分母使用源学期库同层级学生名单，当前统计学期使用当前总览卡片同一名单。“较上学期”固定等于当前行覆盖率减去紧邻上一学期行覆盖率，首页卡片、学校、学院和专业历史明细共用同一结果，不使用隐藏的当前名单回溯基线。挂科学生率分母使用同层级当学期有有效成绩的学生；平均 GPA 只对同层级当学期有加权 GPA 的学生求平均。历史源库文件缺失、`students` 表缺失或组织名称无法映射时，该历史学期返回不可用原因，不回退使用当前 `dim_student`。
...
- 标题：`{专业名称} · 当前有效预警学生名单`。
- 自动带入：学院、专业、专业详情当前选择的统计学期和当前身份；专业不可修改。统计学期仅在固定范围条中说明来源分析上下文，不传给当前预警接口。
...
| DEC-15 | 有效成绩覆盖率统一口径 | 每学期覆盖率=该学期有效成绩人数÷该学期在籍人数；“较上学期”=当前行覆盖率－紧邻上一学期行覆盖率。首页卡片与学校、学院、专业历史明细共用该结果，不另设隐藏比较基线；上一学期无可用值时显示“-” |
